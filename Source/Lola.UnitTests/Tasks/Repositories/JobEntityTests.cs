...
public class JobEntityTests {
    private readonly IJobHandler _mockJobHandler;
    private readonly IMap _mockContext;

    public JobEntityTests() {
        _mockJobHandler = Substitute.For<IJobHandler>();
        _mockContext = Substitute.For<IMap>();
        _mockContext.GetRequiredValueAs<IJobHandler>(nameof(JobHandler)).Returns(_mockJobHandler);
    }
